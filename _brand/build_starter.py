...
.st-next,.st-fail{margin:.85rem 0 0;font-size:.92rem;line-height:1.5;padding:.85rem .95rem;border-radius:12px}
.st-next{background:var(--well);border:1px solid var(--line);color:var(--ink)}
.st-fail{background:var(--card);border:1px solid var(--line-strong);color:var(--ink)}
.st-next a,.st-fail a{color:inherit;text-underline-offset:3px}
.st-next[hidden],.st-fail[hidden]{display:none}
...
<main id="main" class="wrap" style="padding-top:2.2rem;padding-bottom:3rem;max-width:52rem">
  ...
  <div class="st-panel">
    ...
    <!-- Shown only after a successful build: the moment someone has just watched the
    ...
    <p class="st-fail" id="st-fail" hidden>Email me at
      <a href="mailto:colin@automatedworkflowllc.com?subject=Starter%20workbook%20wouldn%27t%20build">colin@automatedworkflowllc.com</a>
      and I&rsquo;ll send you the same workbook myself.</p>
  </div>
...
document.getElementById('st-build').addEventListener('click', function(){
  ...
  var next = document.getElementById('st-next');
  var fail = document.getElementById('st-fail');
  try {
    ...
    // e.message goes through textContent, never innerHTML — it is browser-supplied
    // text and must not be able to inject markup. The actionable address lives in
    // static markup instead, so the failure path always offers a real next step.
