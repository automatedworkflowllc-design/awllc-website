...
/* ---- findings, ranked -------------------------------------------------
   The difference between a dashboard and a tool: a dashboard is looked at, a findings
   list is acted on. Every row is a claim, the number that justifies it, and the move it
   implies -- in that order, ranked by dollars at stake, so the top row is the call to
   make first. Numbered rather than bulleted because the ORDER is the product here.
   Deliberately not styled as an alert: these are facts about the file, not alarms. */
.dfind{margin:0 0 1.05rem;padding:.95rem 1.1rem;border:1px solid var(--d-line2);
border-radius:11px;background:var(--d-card)}
.dfind-h{margin:0 0 .6rem;font-size:.82rem;font-weight:700;letter-spacing:.04em;
text-transform:uppercase;color:var(--d-ink)}
.dfind-l{margin:0;padding:0;list-style:none;display:flex;flex-direction:column;gap:.7rem}
.dfind-l li{display:flex;gap:.65rem;align-items:flex-start}
.dfind-n{flex:0 0 auto;width:1.35rem;height:1.35rem;border-radius:50%;
background:var(--d-well);border:1px solid var(--d-line2);font-family:var(--d-mono);
font-size:.7rem;font-weight:700;color:var(--d-ink);display:flex;align-items:center;
justify-content:center;margin-top:.05rem}
.dfind-b{min-width:0}
.dfind-t{margin:0;font-size:.93rem;font-weight:650;color:var(--d-ink);text-wrap:pretty;
letter-spacing:-.005em}
.dfind-e{margin:.15rem 0 0;font-size:.84rem;color:var(--d-soft);text-wrap:pretty}
.dfind-f{margin:.7rem 0 0;font-size:.78rem;color:var(--d-soft);text-wrap:pretty;
border-top:1px solid var(--d-line);padding-top:.55rem}
    ...
    <div class="dmain">
      ...
      <div class="dbody">
        <section id="view-dashboard" role="tabpanel" aria-labelledby="tab-dashboard" tabindex="0">
          ...
          <!-- Ranked findings. Sits between the summary and the ask because it IS the payoff:
               the summary says what happened, this says what to do about it and in what order.
               Rendered by renderDashboard(), so it follows whichever book is loaded -- sample
               or real -- and needs no separate clearing on "Back to sample". -->
          <div class="dfind" id="d-findings" hidden></div>
...
__SHARED_INTAKE__
  ...
  /* A customer name is a GROUPING KEY, and untrimmed it silently stops being one.
     Measured 2026-08-13 on a deliberately messy paste: one account appeared as both
     "Bell Ridge HOA" and "Bell Ridge HOA " (one trailing space). That split it into two
     customers holding 38% and 9% of the book instead of one holding 47% -- which pushed it
     under the 40% threshold and SUPPRESSED A TRUE CONCENTRATION FINDING. The page showed
     nothing, which is the worst failure available here, and HTML collapses the space so the
     reader could not have seen why. It also inflated the active-customer count.
     Trim, collapse internal runs, and strip a trailing comma; nothing else is touched, because
     beyond this every "cleanup" starts guessing that two spellings are one business. */
  ...
  function adopt(raw){
    ...
    var recv = (raw.receivables || []).map(function (r, i) {
      ...
      /* Same normalisation as the CSV/xlsx path. This one takes AW_DASHBOARD_DATA, so a
         client feeding real data in gets the same grouping guarantee as a pasted file. */
      return { customer: cleanName(r && r.customer),
  ...
  function rowsHTML(items, withDraft){
    /* "Oldest open — call first" is a claim about ONE invoice, so exactly one row may carry it.
       It used to be attached to any invoice past 90 days, which meant a book with two such
       invoices printed "oldest open — call first" twice and disagreed with itself on screen --
       and with the summary above, which names a single account. That is precisely the
       self-contradiction this product is sold to find, on the page every outreach email points
       at. Found 2026-08-13 by pasting a deliberately messy book rather than the sample.
       `items` arrives sorted oldest-first, so the oldest overdue row is the first `late` one. */
    var oldestIdx = -1;
    items.forEach(function (r, i) {
      if (oldestIdx === -1 && r.days !== null && r.days > 30) { oldestIdx = i; }
    });
    return items.map(function (r, i) {
      var late = r.days !== null && r.days > 30;
      var note = r.days === null
        ? 'no date on this invoice — age unknown'
        : (late ? (r.days + 'd overdue · ' + (i === oldestIdx ? 'oldest open — call first'
                   : r.days > 90 ? 'past 90 days' : r.days > 60 ? 'crossed 60 days'
                   : 'due ' + fmtDay(r.due)))
  ...
  function bookFromCSV(text, fileName){
    ...
    rows.forEach(function (r) {
      ...
      if (si >= 0 && n > 0) {
        ...
        if (!settled) {
          open.push({ customer: ci >= 0 ? cleanName(r[ci]) : 'Unnamed account',
  ...
  /* Findings, ranked by dollars at stake.

     RULES THIS PANEL LIVES BY, because it is the most quotable thing on the page and
     therefore the easiest place to accidentally lie:
       - Every finding is arithmetic over the loaded book. Nothing is estimated, and no
         multiplier, benchmark or industry rate appears anywhere in here. (Contrast
         neverInvoiced, which is a sample-only fixed proportion and is gated as such.)
       - Nothing asserts a cause. "One account holds 62% of what you are owed" is a fact
         about the file; "you are over-reliant on them" is a judgement about their
         business that a spreadsheet cannot support.
       - An empty list renders NOTHING. It must never print "no issues found", because
         "we looked and found nothing" and "we could not look" would then be identical on
         screen -- the failure this whole site is pointed at. The limits panel states what
         could not be checked; that is where an absence belongs.
       - Thresholds are stated in the text itself, so a reader can disagree with one. */
  function findingsPanel(){
    var host = el('d-findings');
    if (!host) { return; }
    var f = [];

    /* 1. The single oldest genuinely-overdue invoice. overdue[] is already filtered to
          days > 30, and undated invoices are excluded from it upstream -- an undated
          invoice is unknown, not old. */
    var late = overdue[0];
    if (late) {
      f.push({ w: late.amount,
        t: 'Call ' + esc(late.customer) + ' first.',
        e: money(late.amount) + ' is ' + late.days + ' days past due &mdash; the oldest ' +
           'thing you are owed. Nothing else on this list is worth more per phone call.' });
    }
    ...
    if (acctCount > 1 && arTotal > 0) {
      ...
      if (topName && share >= 40) {
        f.push({ w: topVal,
          ...
          /* "sits with one account, across N accounts in total" read as though the money were
             SPREAD across N -- the exact opposite of the finding. Say the count first, as
             context, so the share lands unambiguously on the single name. */
          e: 'You have ' + acctCount + ' accounts on the book, and ' + money(topVal) +
             ' of the ' + money(arTotal) + ' outstanding sits with this one. ' +
             'Whatever they do with their payment schedule, you do too.' });
      }
    }

    /* 3. How much of the book is already late, as distinct from finding #1, which is one
          invoice. A portfolio can be healthy with one bad invoice in it and unhealthy
          with none older than the rest.

          Suppressed when there is exactly ONE overdue invoice, because then this is not a
          second finding -- it is finding #1 restated as a percentage, and a list that says
          the same thing three ways is the padding this panel exists to avoid. Caught by a
          deliberate two-account control file, where the same $8,000 invoice generated
          three of four rows. */
    if (arTotal > 0 && overdueTotal > 0 && overdue.length > 1) {
      var oShare = (overdueTotal / arTotal) * 100;
      if (oShare >= 25) {
        f.push({ w: overdueTotal,
          t: Math.round(oShare) + '% of your outstanding money is already past 30 days.',
          e: money(overdueTotal) + ' of ' + money(arTotal) + ', across ' + overdue.length +
             ' invoice' + (overdue.length === 1 ? '' : 's') + '. Shown because it is over a ' +
             'quarter of the book &mdash; below that it is ordinary, above it is a pattern.' });
      }
    }

    /* 4. Pace against the SAME day of the prior month. Never against a whole prior month,
          which would report a fake collapse on the 3rd of every month. */
    if (pmtd > 0 && Math.abs(mtdDelta) >= 15) {
      var down = mtdDelta < 0;
      f.push({ w: Math.abs(mtd - pmtd),
        t: 'This month is running ' + Math.abs(Math.round(mtdDelta)) + '% ' +
           (down ? 'behind' : 'ahead of') + ' last month.',
        e: money(mtd) + ' so far against ' + money(pmtd) + ' by the same day last month' +
           (down ? '. Worth knowing now rather than at month end, while there are still ' +
                   'days left to change it.'
                 : '. If it holds, plan for the work rather than being surprised by it.') });
    }

    if (!f.length) { host.hidden = true; host.innerHTML = ''; return; }

    f.sort(function (a, b) { return b.w - a.w; });

    host.hidden = false;
    host.innerHTML =
      '<p class="dfind-h">What to do first &mdash; ' + f.length + ' finding' +
      (f.length === 1 ? '' : 's') + ', biggest first</p>' +
      '<ol class="dfind-l">' +
      f.map(function (x, i) {
        return '<li><span class="dfind-n">' + (i + 1) + '</span><div class="dfind-b">' +
               '<p class="dfind-t">' + x.t + '</p>' +
               '<p class="dfind-e">' + x.e + '</p></div></li>';
      }).join('') +
      '</ol>' +
      '<p class="dfind-f">Ranked by the amount of money each one is about. Every line is ' +
      'arithmetic on the file you loaded &mdash; no estimates, no industry averages, and ' +
      'nothing inferred about why.</p>';
  }
