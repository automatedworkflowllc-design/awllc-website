...
/* A MONEY COLUMN CAN NEVER BE THE JOIN KEY. An amount is not an identity.
   Measured on the shipped page 2026-08-19, with work and invoice files whose ID
   columns shared nothing and whose amounts were all distinct: this joined
   Amount <-> Amount and then reported "$800 uninvoiced" keyed as `800`, while
   SILENTLY HIDING a genuinely unbilled $650 job -- because an unrelated invoice
   happened to be for $650. A false all-clear over real money, plus a false
   accusation, from one wrong key.
   The uniqueness filter below does NOT cover this: it only rejected the amount
   column in earlier testing because that fixture happened to repeat a value.
   With all-distinct amounts, which is the ordinary case, it sails through.
   /check/ took the same defect and the same fix on 2026-08-17; this page never
   got the guard. "These files do not line up" is the honest answer and it is
   strictly better than a confident reconciliation on the wrong column. */
...
function detectJoin(a, b){
  ...
  for(var i=0;i<a.header.length;i++){
    if(MONEYISH.test(a.header[i]||'')) continue;
    ...
    for(var j=0;j<b.header.length;j++){
      if(MONEYISH.test(b.header[j]||'')) continue;
