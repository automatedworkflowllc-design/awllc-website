...
<main id="main" class="wrap" style="padding-top:2.2rem;padding-bottom:3rem;max-width:52rem">
  ...
  <section id="dc-report" aria-live="polite">
    ...
    <div class="dc-cta">
      ...
      <!-- Filled in by run() ONLY for a visitor's own file. mailto:, never a form POST: this
           page promises the network tab stays empty, and a POST would make that false. -->
      <div id="dc-send" hidden></div>
...
function run(){
  ...
  if(current.isSample){
    var sh = document.getElementById('dc-send');
    if(sh){ sh.hidden = true; sh.innerHTML = ''; }
  } else {
    sendBlock(distinctRaw, realCount, groups.length);
  }
...
/* Counts only -- never a customer name. The whole promise of this page is that the list
   stays on the visitor's machine, and a summary naming "Dealership LLC vs dealership inc"
   would leak exactly what they came here not to hand over. */
function sendBlock(distinctRaw, realCount, collisions){
  var host = document.getElementById('dc-send');
  if(!host) return;
  if(!collisions){ host.hidden = true; host.innerHTML = ''; return; }
  var body = 'Hi Colin,\n\nI ran the duplicate customer finder on my own list. It found:\n\n' +
             '- ' + distinctRaw + ' distinct spellings\n' +
             '- ' + collisions + ' collision' + (collisions===1?'':'s') + '\n' +
             '- ' + realCount + ' likely real customers\n\n' +
             'I have not sent you the list itself.\n\n[Anything you want to add]\n';
  var href = 'mailto:colin@automatedworkflowllc.com' +
             '?subject=' + encodeURIComponent('Duplicate customer finder - what it found on my list') +
             '&body=' + encodeURIComponent(body);
  host.hidden = false;
  host.innerHTML =
    '<p style="margin:.9rem 0 .5rem;font-size:.85rem">Or send Colin just these three numbers &mdash; ' +
    'opens your own email app, already written. Your list stays in this browser; nothing is attached.</p>' +
    '<a class="btn" id="dc-mail" href="' + href + '">Email these numbers to Colin</a>';
}
...
function load(name, text, isSample){
  ...
  current = { name: name, header: header, body: body, isSample: !!isSample };
...
document.getElementById('dc-sample').addEventListener('click', function(){ load('sample-customers.csv', SAMPLE, true); });
