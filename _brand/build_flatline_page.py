...
.fl-check{border:1px solid var(--line);border-radius:.6rem;padding:.8rem 1rem;background:var(--bg-soft,var(--well,#ECE8DB))}
...
.fl-run{border:1px solid var(--line);border-radius:12px;background:var(--bg-soft,var(--well,#ECE8DB));padding:1.3rem;margin:1.6rem 0 0}
...
<div class="pe">
  ...
  <p style="color:var(--ink-soft);max-width:40rem">Six shapes of the same problem. If any of these
  sounds familiar, it is worth twenty minutes.</p>

  <div class="fl-uses">
    <div class="fl-use">
      <h3>The interface that quietly went dead</h3>
      <p class="sym">Twenty-eight hover states that no longer change anything, because one colour
      token got aliased to another. The types check. Every component is correct on its own. A
      screenshot of any single button looks right. The only symptom is that the UI feels dead.</p>
      <p class="fix"><b>scan</b> &mdash; point it at the resolved values rather than the source.
      Twenty-eight states that all compute to the same colour is a constant wearing twenty-eight
      names, and that is the one thing this tool is built to see. Nothing else in a front-end
      toolchain is looking for <em>sameness</em>: a linter checks shape, a type checker checks
      types, a snapshot test checks that today matches yesterday &mdash; and yesterday was already
      broken.</p>
    </div>
  ...
  <div class="fl-run">
    ...
    <p style="color:var(--ink-soft);margin-top:.3rem">Free, MIT licensed, 131 tests. It reads; it
